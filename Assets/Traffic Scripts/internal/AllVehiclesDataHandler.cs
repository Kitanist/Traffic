...
namespace Gley.TrafficSystem.Internal
{
    ...
    internal class AllVehiclesDataHandler
    {
        ...
        {
            Debug.Log(GetVehicle(vehicleIndex).excluded);
        ...
        internal void ActivateVehicle(VehicleComponent vehicle, Vector3 position, Quaternion vehicleRotation, Quaternion trailerRotation)
        {
            ...
            if (API.IsInitialized())
            {
             ...
             //           rendererList = carHolder.GetComponentsInChildren<MeshRenderer>(true).ToList(); // bunun çocuğunun çocuğunu kapa
             ...
             //           }
             //   }
             //   //  Debug.LogError("SAyııı:"+rendererList.Count);
             //   foreach (var renderer in rendererList)
             //   {
             //       renderer.enabled = TrafficManager.Instance.trafficUIManager.isCarsActive;
             //   }
        ...
        internal void UpdateVehicleLights(bool on)
        {
            for (int i = 0; i < GetAllVehicles().Length; i++)
            {
                Debug.LogError("UpdateVehicleLights:" + on);
