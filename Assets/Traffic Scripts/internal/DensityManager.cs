...
namespace Gley.TrafficSystem.Internal
{
    ...
    internal class DensityManager
    {
        ...
        public readonly IdleVehiclesDataHandler _idleVehiclesDataHandler;
        private readonly PositionValidator _positionValidator;
        private readonly GridDataHandler _gridDataHandler;
        public readonly WaypointManager _waypointManager;
        ...
        internal void UpdateVehicleDensity(Vector3 playerPosition, Vector3 playerDirection, Vector3 activeCameraPosition)
        {
            if (_currentNrOfVehicles < _maxNrOfVehicles)
            {
                ...
                BeginAddVehicleProcess(playerPosition, playerDirection, gridCell, false); // gerekli durumlarda true ve false olarak kullanılmalı.
            }
        }
        ...
        internal void DisableAreaWaypoints(Area area)
        {
            ...
            for (int i = neighbors.Count - 1; i >= 0; i--)
            {
                ...
                for (int j = 0; j < cell.TrafficWaypointsData.Waypoints.Count; j++)
                {
                    ...
                    {
                       var trafficWaypointsDatas = MonoBehaviourUtilities.GetOrCreateObjectScript<TrafficWaypointsData>(TrafficSystemConstants.PlayHolder, false);
                        trafficWaypointsDatas.DisabledWaypoints.Add(waypointIndex);
                        _waypointManager.AddDisabledWaypoint(waypointIndex,true);
                    }
                }
            }
        }
        ...
        public List<CellData> CellDatass;
        /// <summary>
        ...
        {
            Profiler.BeginSample("LoadVehiclesBoon");
        ...
        internal void LoadınitialVehiclesButCamChanced( Vector3 playerPosition, Vector3 playerDirection, Vector3 activeCameraPosition)
        {
           
            var gridCells = new List<CellData>
            {
                _gridDataHandler.GetCell(activeCameraPosition.x, activeCameraPosition.z)
            };
        ...
        private void BeginAddVehicleProcess(Vector3 playerPosition, Vector3 playerDirection, CellData gridCell, bool ignorLOS)
        {
            if (_requestedVehicles.Count == 0)
            {
                ...
                // Debug.Log("Şu an mı eklendi   "   + ignorLOS);
        ...
        private void AddVehicleOnArea(Vector3 playerPosition, Vector3 playerDirection, CellData gridCell, bool ignorLOS)
        {
            ...
            // get neighbor cell waypoint gereğinden fazla index döndryor onu bi elden geçirmek laızm
            ...
            if(freeWaypointIndex >= _trafficWaypointsDataHandler._trafficWaypointsData._allTrafficWaypoints.Length)
            {
                Debug.Log("Geçersiz Poizsyona erişim isteği.  Erişilmek istenen waypoint :" + freeWaypointIndex.ToString());
                return;
            }
            ...
            if (freeWaypointIndex == TrafficSystemConstants.INVALID_WAYPOINT_INDEX)
            {
               ...
               // Debug.Log("Density: No free waypoint found");
                ...
                return;
                          ...
                          //  Debug.Log("Ekledim kral     " + ignorLOS );
