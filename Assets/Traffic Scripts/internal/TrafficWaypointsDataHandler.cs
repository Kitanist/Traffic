...
namespace Gley.TrafficSystem.Internal
{
    ...
    {
        public readonly TrafficWaypointsData _trafficWaypointsData;


        public TrafficWaypointsDataHandler(TrafficWaypointsData data)
        {
        ...
        {
            if (_trafficWaypointsData.AllTrafficWaypoints == null || _trafficWaypointsData.AllTrafficWaypoints.Length == 0)
            {
                Debug.LogError("Waypoint verisi yüklenemedi! Build sýrasýnda eksik olabilir.");
            }
            // Debug.Log($"GetWaypoint çaðrýldý: Index = {waypointIndex}, Max = {_trafficWaypointsData.AllTrafficWaypoints.Length}");

            if (waypointIndex < 0 || waypointIndex >= _trafficWaypointsData.AllTrafficWaypoints.Length)
            {
                Debug.LogError($"HATA: Geçersiz waypoint index: {waypointIndex} (Max: {_trafficWaypointsData.AllTrafficWaypoints.Length})");
                return null; // Hata durumunda null dönelim ki çökme olmasýn.
            }
