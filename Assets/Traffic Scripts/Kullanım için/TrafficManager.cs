...
#pragma warning restore 0649
        ...
        internal void Initialize(Transform[] activeCameras, int nrOfVehicles, VehiclePool vehiclePool, TrafficOptions trafficOptions)
        {
            ...
            // Load intersection data.   burayı kontrol etmem lazım
...
#endif
            _initialized = true;
           // trafficUIManager =  GameObject.FindAnyObjectByType<TrafficUIManager>();
        }
        #endregion
        ...
        internal void RemoveVehicle(GameObject vehicle,bool isFinish)
        {
            if (!_initialized)
                return;
            ...
            if (index != -1)
            {
                RemoveVehicle(index, true,isFinish);
            }
            else
            {
        ...
        internal void RemoveVehicle(List<GameObject> vehicles)
        {
        ...
        internal int GetActiveSquaresLevel()
        {
            return _activeSquaresLevel;
        }
        ...
        internal int GetClosestWaypoint(Vector3 position)
        {
        ...
        internal int GetClosestWaypointDir(Vector3 position, Vector3 direction)
        {
            CellData cell = _gridDataHandler.GetCell(position);
        ...
        private void NewVehicleAdded(int vehicleIndex)
        {
          ...
          // Debug.Log($"TargetWaypoint {WaypointManager.GetTargetWaypointIndex(vehicleIndex)}");
        ...
        internal void RemoveVehicle(int vehicleIndex, bool force,bool isFinish)
        {
            if (!_initialized)
                return;
            ...
            if(vehicleIndex < 1000 && vehicleIndex > 848  && !isFinish )
            {
                return;
            }
        ...
        public void ChangeCamAndRemoveCarsForFatihBrother()
        {
            
            DensityManager.LoadınitialVehiclesButCamChanced(_activeCameras[_activeCameraIndex].position, _activeCameras[_activeCameraIndex].forward, _activeCameraPositions[_activeCameraIndex]);
            Debug.Log(_activeCameras[_activeCameraIndex].position  ) ;
        }
            ...
            for (int i = 0; i < _nrOfVehicles; i++)
            {
                ...
                if (!_vehicleRigidbody[i].IsSleeping())
                {
                    ...
                    if (_vehicleNeedWaypoint[i] == true)
                    {
                        ...
                        DrivingAI.WaypointRequested(i, _vehicleType[i], _clearPath);  // waypoint isteniyo
                    }
            ...
            if (_vehicleReadyToRemove[_indexToRemove] == true)
            {
                if (_excludedVehicles[_indexToRemove] == false || (_excludedVehicles[_indexToRemove] == true && _addExcludedVehicle[_indexToRemove] == true))
                {
                    if (_vehicleRigidbody[_indexToRemove].gameObject.activeSelf)
                    {
                        ...
                        {
                            RemoveVehicle(_indexToRemove, false,false);
                        }
                    }
                }
            }
