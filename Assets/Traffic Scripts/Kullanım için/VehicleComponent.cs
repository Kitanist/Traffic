    ...
    public class VehicleComponent : MonoBehaviour, ITrafficParticipant
    {
        ...
        private void OnTriggerStay(Collider other)
        {
           ...
           if (other.CompareTag("Slower"))
           {
               ...
               TrafficLightsController light = other.GetComponentInParent<TrafficLightsController>();
               if (light != null && light.CurrentState == TrafficLightState.Red)
               {
            ...
            if (other.CompareTag("Stopper"))
           {
               //Debug.Log("stay");
               TrafficLightsController light = other.GetComponentInParent<TrafficLightsController>();
             //  Debug.Log($"Light State: {(light.CurrentState == TrafficLightState.Red ? "RED" : "GREEN")}");

                if (light != null && light.CurrentState == TrafficLightState.Red)
                {
                    API.AddDrivingAction(ListIndex, DriveActions.StopTemp, true);
                  //  Debug.Log("Adding STOP action.");
                    return;
                }
                if (light != null && light.CurrentState == TrafficLightState.Yellow)
                {
                    API.AddDrivingAction(ListIndex, DriveActions.Forward, true);
                 //   Debug.Log("Adding FORWARD action.");
                }
                if (light != null && light.CurrentState == TrafficLightState.EmergencyStop)
                {
                    API.AddDrivingAction(ListIndex, DriveActions.StopTemp, true);
                }
                if (light != null && light.CurrentState == TrafficLightState.FlashingRed1s)
                {
                    SetMaxSpeed(50f);
                }
                if (light != null && light.CurrentState == TrafficLightState.FlashingYellow1s)
                {
                    SetMaxSpeed(70f);
                }
                if (light != null && light.CurrentState == TrafficLightState.FlashingRed2s)
                {
                    SetMaxSpeed(50f);
                }
                if (light != null && light.CurrentState == TrafficLightState.FlashingYellow2s)
                {
                    SetMaxSpeed(70f);
                }
        ...
        internal void SetMaxSpeed(float newSpeed)
        {
#if DOTWEEN_PRESENT
    float currentSpeed = _maxSpeed;
    float targetSpeed = newSpeed < 5 ? 0 : newSpeed;

    float speedDelta = Mathf.Abs(targetSpeed - currentSpeed);
    float tweenDuration = Mathf.Clamp(speedDelta * 0.05f, 0.2f, 3f);
    Ease easeType = (targetSpeed > currentSpeed) ? Ease.InCubic : Ease.OutCubic;

    DOTween.To(() => _maxSpeed, x => _maxSpeed = x, targetSpeed, tweenDuration)
           .SetEase(easeType);
#else
            // DOTween yoksa direkt atama yapıyoruz
            _maxSpeed = (newSpeed < 5) ? 0 : newSpeed;
          //  Debug.LogWarning("DOTween aktif değil, hız direkt atandı.");
#endif
        }
